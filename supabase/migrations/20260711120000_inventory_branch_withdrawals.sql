-- Branch 2 withdrawal batch header + atomic IN stock RPC

CREATE TABLE IF NOT EXISTS public.inventory_branch_withdrawals (
...
CREATE INDEX IF NOT EXISTS idx_inventory_branch_withdrawals_created_at
...
DROP POLICY IF EXISTS "Allow authenticated users to view branch withdrawals"
  ON public.inventory_branch_withdrawals;

CREATE POLICY "Allow authenticated users to view branch withdrawals"
  ON public.inventory_branch_withdrawals
  FOR SELECT
  TO authenticated
  USING (true);
