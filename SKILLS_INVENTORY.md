...
| Inventory Count Policy | เมื่อแก้ระบบตรวจนับหรือรายการสั่งซื้อ | `exact_count` คิดรวม accuracy; `sufficiency_check` ไม่คิดรวมคะแนนและใช้ manual `order_qty` |
...
| Local Events Market Context | เมื่อพัฒนา Market Insights | ดึง `local_events` ผ่าน `fetchUpcomingLocalEvents()` และสรุปด้วย `buildLocalEventsContext()`; query failure ต้อง fallback เป็น `[]` |
| Dashboard Query Consolidation | เมื่อ dashboard ใช้ช่วงสัปดาห์และเดือนที่ซ้อนกัน | ใช้ `getDashboardShiftQueryPlan()` รวม Supabase `shifts` query แล้ว `splitDashboardShiftsByRange()` คืน payload เดิม |
| Inventory Route Bundle Split | เมื่อ modal/chart/tool ไม่จำเป็นตอน initial route load | ใช้ `next/dynamic` และ intent preload (`onMouseEnter` / `onFocus`) โดยไม่ mount hidden modal surfaces ตั้งแต่แรก |
| Inventory Row Containment | เมื่อตาราง inventory มีหลายแถวและ cell edits ต้องลื่น | ใช้ `.bb-inventory-row-containment` เพื่อแยก layout/paint work โดยคง spreadsheet behavior และ realtime freshness |
