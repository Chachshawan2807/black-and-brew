...
| :--- | :--- | :--- | :--- |
| **Gemini API** | Multi-modal generation and tool use via Google Gen AI SDK. | การสร้างเนื้อหาแบบหลายรูปแบบและการใช้เครื่องมือผ่าน SDK ของ Google | `.agents/skills/gemini-api/` |
...
| **Python Runtime** | Versatile programming language for AI and scripting. | ภาษาโปรแกรมที่หลากหลายสำหรับการประมวลผล AI และสคริปต์ | `v3.14.3` |
...
| **Mem0** | Long-term AI memory layer — stores user preferences & decisions in local DB. | ระบบความจำระยะยาวสำหรับ AI — บันทึก Preference และการตัดสินใจลงไฟล์ Local DB | `.antigravity/tools/memory-engine/mem0` |
...
| **Token Monitoring** | Token usage and balance verification via local script. | ตรวจสอบปริมาณ Token ที่ใช้ไปและคงเหลือผ่านสคริปต์ | `.antigravity/tools/monitor/check-budget.py` |
| **Token Awareness** | Mandatory token status notification before major Audit tasks. | แจ้งจำนวน Token ที่เหลืออยู่ให้ทราบเสมอก่อนเริ่มงาน Audit ชุดใหญ่ | `Protocol Rule (Ref: check-budget.py)` |