...
- Removed stale keeper language that claimed `actions/tools/` was retained for Vitest `read-table-preset.test.ts`.
- Maintenance reads use `compute-upcoming-maintenance.ts`; proactive insights uses inline query limits (no AI gateway).
- Recorded **DEC-091** in `docs/memory.md`; updated **DEC-089** to reference full removal.
...
- Clarified AI chat retirement: production chat route removed (see DEC-091 for full tools/gateway removal).
...
- Removed `POST /api/chat` and the full chat AI stack: `src/lib/agents/`, `src/lib/ai-data-gateway.ts`, `src/app/actions/tools/`, chat UI, external search client, chat formatters, and related Vitest suites.
...
- Removed automatic parcel tracking UI and carrier API timeline; bean orders use manual ship + delivery confirm only.
