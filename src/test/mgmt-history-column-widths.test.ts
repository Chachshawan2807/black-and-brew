...
describe('computeMgmtHistoryColumnWidths', () => {
  test('sizes columns from the longest row value within min/max bounds', () => {
    const widths = computeMgmtHistoryColumnWidths(columns, [
      ...
      },
      {
        ...
        dateRange: '01/08/2026 → 05/08/2026',
        shiftTypeLabel: 'เปลี่ยนกะ 14:00',
        remark: 'ไปธุระที่กรุงเทพฯ',
      },
    ]);

    expect(widths.employee_name).toBeGreaterThan(72);
    ...
    expect(widths.remark).toBeGreaterThan(56);
  ...
  test('uses header labels when history is empty', () => {
    ...
    expect(widths.employee_name).toBeGreaterThanOrEqual(72);
    expect(widths.date_range).toBeGreaterThanOrEqual(108);
    expect(widths.shift_type).toBeGreaterThanOrEqual(76);
    expect(widths.remark).toBeGreaterThanOrEqual(56);
  });
  ...
  test('caps very long remarks so the table does not become excessively wide', () => {
    ...
    expect(widths.remark).toBeLessThanOrEqual(288);
  });
});
...
describe('formatMgmtHistoryDateRange', () => {
  ...
  test('shows an arrow range for multi-day entries', () => {
    expect(formatMgmtHistoryDateRange('2026-08-01T09:00:00', '2026-08-05T18:00:00')).toBe(
