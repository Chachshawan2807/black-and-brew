...
describe('branch withdraw dialogs', () => {
  test('all modal dialogs center with m-auto and respect mobile viewport height', () => {
    ...
    expect(branchWithdrawClient).toMatch(/ref=\{historyLineDialogRef\} className=\{BRANCH_WITHDRAW_DIALOG_WIDE_CLASS\}/);
    expect(branchWithdrawClient).toMatch(/ref=\{detailDialogRef\} className=\{BRANCH_WITHDRAW_DIALOG_NARROW_CLASS\}/);
