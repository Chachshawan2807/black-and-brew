...
export function RouteLoadingSkeleton({ label }: RouteLoadingSkeletonProps) {
  return (
    <div className="min-h-screen bg-background p-4 text-foreground md:p-10">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-5">
        <div className="h-16 rounded-3xl border border-border bg-card" />
        ...
        <div className="rounded-3xl border border-border bg-card p-4">
          <div className="mb-4 h-4 w-48 rounded-full bg-muted" />
          <div className="space-y-3">
            {Array.from({ length: 7 }, (_, index) => (
              <div key={index} className="h-10 rounded-2xl bg-muted" />
            ))}
          </div>
        </div>
