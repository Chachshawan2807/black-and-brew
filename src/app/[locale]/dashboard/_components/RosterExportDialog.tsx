...
function buildDefaultSelectedIds(
  profiles: RosterExportProfile[],
  initialSelectedId: string | null,
): Set<string> {
...
type RosterExportDialogPanelProps = {
  orderedProfiles: RosterExportProfile[];
  initialSelectedId: string | null;
  onClose: () => void;
  onExportSelected: (staffIds: string[]) => void;
  onExportAll: () => void;
...
function RosterExportDialogPanel({
  orderedProfiles,
  initialSelectedId,
  onClose,
  onExportSelected,
  onExportAll,
}: RosterExportDialogPanelProps) {
  const [selectedIds, setSelectedIds] = useState(() =>
    buildDefaultSelectedIds(orderedProfiles, initialSelectedId),
  );

  const allSelected =
    orderedProfiles.length > 0 && selectedIds.size === orderedProfiles.length;
  ...
  const toggleAll = () => {
    setSelectedIds(
      allSelected ? new Set() : new Set(orderedProfiles.map((profile) => profile.id)),
    );
  };
...
export function RosterExportDialog({
  ...
  const selectionResetKey = open
    ? `${initialSelectedId ?? ''}:${orderedProfiles.map((profile) => profile.id).join(',')}`
    : 'closed';
