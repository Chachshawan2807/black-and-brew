...
Loaded eagerly by SKILL.md Step 3 whenever the catalog pick is `carnival`. The default palette ("Cold Snap") and font stack live in the **Palette drops** section below (and axis values in [`theme-axes.md`](../theme-axes.md)); this file carries the **palette drops** (six variants) plus the rules that tokens cannot encode.
