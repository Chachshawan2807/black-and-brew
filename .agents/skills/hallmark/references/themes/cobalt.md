...
Loaded eagerly by SKILL.md Step 3 whenever the catalog pick is `cobalt`. The OKLCH palette + font stack live in the **Palette** and **Typography** sections below (and axis values in [`theme-axes.md`](../theme-axes.md)). Canonical shape: API product landing with a request/response hero (code-as-hero, hairline rules, single cobalt signal).
...
Plus the Space Grotesk + Inter + JetBrains Mono link and the small reveal/type-in script. Mirror the request/response hero pattern described in this file (code-as-hero, hairline frame, single cobalt signal).
