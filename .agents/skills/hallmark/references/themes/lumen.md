...
Loaded eagerly by SKILL.md Step 3 whenever the catalog pick is `lumen`. Palettes + font stack live in the **Palette drops** section below (Night default · Day drop) and axis values in [`theme-axes.md`](../theme-axes.md). This file carries the drops + apparatus family + signature moves + motion + anti-patterns.
