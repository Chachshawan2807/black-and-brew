...
Loaded eagerly by SKILL.md Step 3 whenever the catalog pick is `hum`. The palette + font stack live in the **Palette** and **Typography** sections below (and axis values in [`theme-axes.md`](../theme-axes.md)). This file carries signature moves, motion direction, voice fixtures, anti-patterns, and how Hum holds its identity against neighbouring themes. Hum is the catalog's only playful theme — its closest siblings are Coral (single-accent restraint, modern-minimal) and Bloom (warm-light atmospheric), not another playful register.
