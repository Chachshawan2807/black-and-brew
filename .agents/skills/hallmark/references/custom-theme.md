...
2. **Does not save themes for reuse.** A custom run is per-output. The skill does not write back to a shared catalog file. If the user wants a permanent theme, they paste the custom palette into their project's `tokens.css` (or `design.md`) themselves and name it.
